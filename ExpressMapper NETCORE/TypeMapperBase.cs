...
namespace ExpressMapper
{
    public abstract class TypeMapperBase<T, TN> : IMemberConfigParameters
    {
        ...
        public Func<object, object, object> GetNonGenericMapFunc()
        {
            ...
            var methodInfo = customGenericType.GetMethod("MapTo", new Type[] { typeof(T), typeof(TN) });
        ...
        {
            var callSetPropMethod = propertySet is FieldInfo setFieldProperty
                ? Expression.Field(DestFakeParameter, setFieldProperty)
                : Expression.Property(DestFakeParameter, propertySet as PropertyInfo);
            var callGetPropMethod = propertyGet is FieldInfo getFieldProperty
                ? Expression.Field(SourceParameter, getFieldProperty)
        ...
        {
            var getFields = typeof(T).GetFields(BindingFlags.FlattenHierarchy | BindingFlags.Instance | BindingFlags.Public);
            var setFields = typeof(TN).GetFields(BindingFlags.FlattenHierarchy | BindingFlags.Instance | BindingFlags.Public);

            var getProps = typeof(T).GetProperties(BindingFlags.FlattenHierarchy | BindingFlags.Instance | BindingFlags.Public);
            var setProps = typeof(TN).GetProperties(BindingFlags.FlattenHierarchy | BindingFlags.Instance | BindingFlags.Public);
